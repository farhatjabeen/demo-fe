...
export default function Faq({ questions }) {
    ...
    return (
        <div className=' bg-white mb-32'>
            ...
            <div className='relative flex justify-end xl:flex-row md:flex-col md:items-center w-full xl:pr-14 
            xl:py-14 sm:flex-col sm:items-center mt-10'>
                <div className='md:ml-20 mt-4 xl:absolute xl:w-3/6 xl:top-10 xl:left-0 
                md:w-2/3 sm:w-3/4 xl:end-1/3 xl:mr-40 md:flex md:flex-col sm:flex sm:flex-col'>
                    {questions.map((items, idx) => {
                        return (
                            <div className={`${plusMinus === idx ? "xl:h-20" : "xl:h-14"} 
                            mb-6 border-solid border-b border-back-dark`} key={idx}>
                                <div className='flex justify-between '>
                                    <div className='xl:text-xl md:text-base sm:text-sm font-medium text-black-dark' key={idx}>
                                        {items.questions}
                                    </div>
                                    <div>
