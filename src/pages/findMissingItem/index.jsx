...
export default function FindMissingItem() {
  ...
  const handlePageChange = (pageNumber) => {
    if (searchParameters?.location && searchParameters?.location?.length > 0) {
      navigate(
        `/findMissingItem/${searchParameters?.itemName}/${searchParameters?.location}?page=${pageNumber}`
      )
    } else {
  ...
  return (
    <div className="flex flex-col items-center mt-5 mb-20">
      <h1 className="font-bold text-4xl mb-10">Search results</h1>

      <div
        className="h-20 xl:w-9/12 md:w-8/12 sm:w-8/12 rounded-3xl bg-white border border-solid 
          ...
          <form onSubmit={methods.handleSubmit(submitData)} className="w-full flex">
            <div className="flex w-9/12">
              <TextInput
                ...
                placeholder="Search..."
                name="keyword"
                className="ml-3 p-4 placeholder:text-black xl:h-14 sm:h-13 w-96 rounded-2xl border border-solid border-greys"
                autoComplete="off"
                isSearchReport={true}
      ...
      {isLoader ? (
        ...
        <div className="flex flex-wrap justify-center items-center w-full mr-7">
          {ValueByLocation?.list?.length ? (
            ValueByLocation?.list?.map((items, i) => {
              return (
                <div className="sm:w-60 md:w-52 xl:w-80 xl:ml-10 md:ml-5 mt-8 sm:flex sm:items-center">
                  <SearchCards
                    ...
                    idx={i}
                    itemId={items._id}
                    imageName={items.itemImage || ''}
                    itemName={items.itemName}
                    location={items.location}
                    date={items.foundDate}
                    time={items.foundTime}
                  />
                </div>
              )
            })
